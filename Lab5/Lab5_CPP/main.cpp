...
string passGen(string words[], int number){
    ...
    int left = 0;
    int right = psswrd.length() - 1;
    while (left < right){
        char temp = psswrd[left];
        psswrd[left] = psswrd[right];
        psswrd[right] = temp;
        left++;
        right--;
    }
...
//q-3
bool search_letter(char key, string source){
...
int main() {
    ...
    int user_number;
    cout<<"Enter number[3-7]:";
    cin>>user_number;

    while (user_number<3 || user_number>7){
        cout<< "It must be between 3 an 7: ";
        cin>>user_number;
    }

    string initial_psswrd = passGen(words, user_number);
    cout<<"Initial password: "<<initial_psswrd;

    string new_psswrd = rep_with_upper(initial_psswrd);
    cout<<"\nNew password: "<<new_psswrd;

    string final_psswrd = swap_letters(new_psswrd);
    cout<<"\nFinal password: "<<final_psswrd;

    char first_letter;
    cout<<"\nEnter first letter of your name: ";
    cin>> first_letter;

    if (search_letter(first_letter,final_psswrd)){
        cout<< "Key letter exists in the password";
    }else{
        cout<<"Key letter does not exist in the password";
    }
    return 0;
}
